...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $openTeams = Team::where("status", "approved")
            ->get()
            ->filter(function ($team) {
                return $team->members()->where("status", "active")->count() <
                    $team->max_member;
            })
            ->count();
        ...
        $unresolvedLF = LostfoundItem::whereNotIn("status", [
            "resolved",
        ])->count();
        ...
        $teamStatus = [
            ...
            "full" => max(
                0,
                Team::where("status", "approved")->count() - $openTeams,
            ),
        ];
        ...
        $recentTeams = Team::latest("created_at")
            ->take(5)
            ->get()
            ->map(function ($t) {
                return [
        ...
        $recentLF = LostfoundItem::latest("created_at")
            ->take(5)
            ->get()
            ->map(function ($l) {
                return [
