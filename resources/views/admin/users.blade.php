    ...
    <div class="user-management" x-data="userManagement({{ $users->toJson() }})">
        ...
        <div class="summary-cards">
            ...
            <div class="card hover-scale">
                <div class="card-content">
                    ...
                    <span class="card-subtitle" style="color: #FBBF24;">Users with no activity > 3 days <i
            ...
            <div class="filter-actions-group">
                <div class="dropdown-wrapper" x-data="{ open: false }">
                    <button type="button" class="dropdown-btn" @click="open = !open">
                        <span
                            x-text="status === 'Semua Status' ? 'Semua Status' : (status.charAt(0).toUpperCase() + status.slice(1))">Semua
                            Status</span>
                        ...
                        <tr class="table-row-hover" x-init="$nextTick(() => lucide.createIcons())">
                            <td>
                                <div class="user-info">
                                    <div class="avatar-wrapper">
                                        <img :src="`https://ui-avatars.com/api/?name=${user.encoded_name}&background=E2E8F0&color=475569&bold=true`"
                            ...
                            <td>
                                <div class="action-buttons">
                                    ...
                                    <!-- Edit User -->
                                    <button class="action-btn text-primary" @click="openEditModal(user)"
                                        title="Edit Pengguna">
                                    ...
                                    <template x-if="user.status !== 'suspended'">
                                        <button class="action-btn text-warning" @click="openSuspendModal(user)"
                                            title="Suspend Pengguna">
                                            <i data-lucide="user-minus" style="width: 18px; height: 18px;"></i>
                                        </button>
                                    </template>
                                    <template x-if="user.status === 'suspended'">
                                        <button class="action-btn text-success" @click="unsuspendUser(user)"
                                            title="Buka Suspend">
                                    ...
                                    <template x-if="user.status !== 'blocked'">
                                        <button class="action-btn text-danger" @click="openBlockModal(user)"
                                            title="Blokir Pengguna">
                                            <i data-lucide="ban" style="width: 18px; height: 18px;"></i>
                                        </button>
                                    </template>
                                    <template x-if="user.status === 'blocked'">
                                        <button class="action-btn text-success" @click="unblockUser(user)"
                                            title="Buka Blokir">
        ...
        <div class="pagination-container" x-show="totalPages > 1" x-cloak>
            <div class="pagination-info">
                Menampilkan <span class="font-bold" x-text="startIndex + 1"></span> - <span class="font-bold"
                    x-text="Math.min(endIndex, totalUsers)"></span> dari <span class="font-bold" x-text="totalUsers"></span>
                data
            </div>
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <input type="email" x-model="newUser.email" required
                                    placeholder="email@student.polije.ac.id">
                            </div>
                            <div class="form-group">
                                <label>NIM</label>
        ...
        <div class="modal-overlay" x-show="showSuspendModal" x-cloak x-transition>
            <div class="modal-container glass-panel" style="max-width: 450px; padding: 0; overflow: hidden;"
                @click.outside="showSuspendModal = false">
                ...
                <template x-if="selectedUser">
                    <div class="modal-body" style="padding: 12px 24px;">
                        <div class="suspend-intro" style="margin-bottom: 8px;">
                            <p style="font-size: 12px; margin-bottom: 0;">Tetapkan durasi dan alasan suspend. Informasi akan
                                dikirim ke email pengguna.</p>
                        </div>

                        <div class="form-group" style="margin-bottom: 12px;">
                            <label style="font-size: 12px; margin-bottom: 4px; display: block;">Durasi Suspend <span
                                    style="color: #EF4444;">*</span></label>
                            <div class="duration-tabs" style="gap: 6px;">
                                <button type="button" style="padding: 8px 4px; font-size: 12px;"
                                    :class="suspendDuration === '1' ? 'active' : ''" @click="suspendDuration = '1'">1
                                    Hari</button>
                                <button type="button" style="padding: 8px 4px; font-size: 12px;"
                                    :class="suspendDuration === '7' ? 'active' : ''" @click="suspendDuration = '7'">7
                                    Hari</button>
                                <button type="button" style="padding: 8px 4px; font-size: 12px;"
                                    :class="suspendDuration === '30' ? 'active' : ''" @click="suspendDuration = '30'">30
                                    Hari</button>
                                <button type="button" style="padding: 8px 4px; font-size: 12px;"
                                    :class="suspendDuration === 'custom' ? 'active' : ''"
                                    @click="suspendDuration = 'custom'">Custom</button>
                            </div>
                        </div>

                        <div class="form-group" style="margin-bottom: 10px;">
                            <textarea x-model="suspendReason" placeholder="Ketik alasan.."
                                style="min-height: 60px; font-size: 13px; padding: 10px;"></textarea>
                        </div>

                        <div class="form-group" style="margin-bottom: 10px;">
                            <label style="font-size: 12px; margin-bottom: 4px; display: block;">Catatan Internal <span
                                    style="color: #94A3B8;">(Opsional)</span></label>
                            <textarea x-model="internalNotes" placeholder="Catatan internal admin..."
                                style="min-height: 50px; font-size: 13px; padding: 10px;"></textarea>
                        </div>
                        ...
                        <div class="info-alert" style="padding: 8px 12px; margin-top: 4px; gap: 8px;">
                            ...
                            <p style="font-size: 10px; line-height: 1.3;">Akun akan otomatis aktif kembali setelah durasi
                                berakhir. Pengguna akan menerima email.</p>
                        </div>
                    </div>
                </template>
                <div class="modal-footer" style="padding: 8px 24px 20px; border-top: none; display: flex; gap: 12px;">
                    <button @click="showSuspendModal = false" class="btn-cancel-suspend"
                        style="flex: 1; padding: 10px; font-size: 14px;">Batal</button>
                    <button @click="confirmSuspend()" class="btn-confirm-suspend"
                        style="flex: 1; padding: 10px; font-size: 14px;" :disabled="loading">
        ...
        <div class="modal-overlay" x-show="showBlockModal" x-cloak x-transition>
            <div class="modal-container glass-panel" style="max-width: 400px; text-align: center;">
                <div class="modal-body" style="padding: 40px 30px;">
                    ...
                    <p style="color: #64748B; font-size: 14px; margin-bottom: 30px;">Pengguna tidak akan bisa masuk ke
                        aplikasi. Tindakan ini dapat dibatalkan melalui buka blokir.</p>

                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                        <button @click="showBlockModal = false" class="btn-secondary"
        ...
        <div class="modal-overlay" x-show="showUnsuspendModal" x-cloak x-transition>
            <div class="modal-container glass-panel" style="max-width: 400px; text-align: center;">
                <div class="modal-body" style="padding: 40px 30px;">
                    <div
                        style="width: 64px; height: 64px; background: #ECFDF5; color: #10B981; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px;">
                    ...
                    <p style="color: #64748B; font-size: 14px; margin-bottom: 30px;"
                        x-text="`Apakah Anda yakin ingin membuka suspend untuk ${selectedUser?.name}?`"></p>

                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                        <button @click="showUnsuspendModal = false" class="btn-secondary"
                            style="justify-content: center;">Batal</button>
                        <button @click="confirmUnsuspend()" class="btn-primary"
                            style="background: #10B981; justify-content: center;" :disabled="loading">
        ...
        <div class="modal-overlay" x-show="showUnblockModal" x-cloak x-transition>
            <div class="modal-container glass-panel" style="max-width: 400px; text-align: center;">
                <div class="modal-body" style="padding: 40px 30px;">
                    ...
                    <p style="color: #64748B; font-size: 14px; margin-bottom: 30px;"
                        x-text="`Apakah Anda yakin ingin membuka blokir untuk ${selectedUser?.name}?`"></p>

                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                        <button @click="showUnblockModal = false" class="btn-secondary"
                            style="justify-content: center;">Batal</button>
                        <button @click="confirmUnblock()" class="btn-primary"
                            style="background: #10B981; justify-content: center;" :disabled="loading">
                            <span x-show="!loading">Ya, Buka</span>
                            <span x-show="loading">Memproses...</span>
                        </button>
                ...
                <template x-if="selectedUser">
                    <div class="modal-body">
                        <div class="user-profile-header">
                            <img :src="`https://ui-avatars.com/api/?name=${selectedUser.encoded_name}&background=E2E8F0&color=475569&size=100&bold=true`"
                                alt="Avatar" class="user-avatar" style="width: 100px; height: 100px;">
