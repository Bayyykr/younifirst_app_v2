...
@section('content')
    <div class="team-monitoring" x-data="teamMonitoring({
        ...
        <!-- Modals (Top of Scope) -->
        <div x-show="showDetailModal" class="modal-overlay" x-transition:enter="transition-fade"
            x-transition:leave="transition-fade" style="display: none; z-index: 9998;">
            <div class="modal-container detail-modal" @click.away="showDetailModal = false"
                    ...
                    <div class="info-grid">
                        <div class="info-item">
                            ...
                            <div class="leader-profile">
                                <img :src="`https://ui-avatars.com/api/?name=${encodeURIComponent(selectedDetailTeam?.leader_name || 'L')}&background=1E293B&color=fff`"
                                    alt="Leader">
                        ...
                        <div class="info-item">
                            ...
                            <div class="info-value">
                                ...
                                <span
                                    x-text="selectedDetailTeam ? new Date(selectedDetailTeam.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : ''"></span>
                            </div>
                        </div>
                        <div class="info-item">
        ...
        <div x-show="showDeleteModal" class="modal-overlay" x-transition:enter="transition-fade"
            x-transition:leave="transition-fade" style="display: none; z-index: 9999;">
            <div class="modal-container respond-modal" @click.away="showDeleteModal = false"
        ...
        <div x-show="showRespondModal" class="modal-overlay" x-transition:enter="transition-fade"
            x-transition:leave="transition-fade" style="display: none; z-index: 9999;">
        ...
        <div x-show="viewMode === 'dashboard'" x-transition:enter="transition-fade">
                            ...
                            <div class="pending-card">
                                ...
                                <div class="pending-card-mid">
                                    ...
                                    <div class="submitter-info">
                                        Submitted by : <span class="submitter-name"
                                            x-text="pTeam.leader_name || 'Leader'"></span>
                                        <span class="submitter-time"
                                            x-text="' &bull; ' + formatTimeAgo(pTeam.created_at)"></span>
                                    </div>
                                </div>
                                ...
                                <div class="pending-card-right">
                                    <div class="member-count-status">
                                        <div class="avatar-circle">
                                            <img :src="`https://ui-avatars.com/api/?name=${encodeURIComponent(pTeam.leader_name || 'L')}&background=1E293B&color=fff`"
                                                alt="User">
                                        </div>
            ...
            <div class="main-toolbar">
                ...
                <div class="filter-pills">
                    <button class="pill-btn" :class="{ 'active': statusFilter === 'Semua' }"
                        @click="statusFilter = 'Semua'">Semua</button>
                    <button class="pill-btn" :class="{ 'active': statusFilter === 'Open' }"
                                    ...
                                    <div class="cell-members">
                                        <div class="avatar-group">
                                            <template x-for="member in team.top_members">
                                                <img :src="`https://ui-avatars.com/api/?name=${member.encoded_name}&background=F1F5F9&color=475569`"
                                                    alt="Avatar">
                                            </template>
                                        ...
                                        <div class="member-stats">
                                            <span class="count-main">
                                                <strong class="text-blue"
                                                    x-text="`${team.active_count}/${team.max_member}`"></strong>
                                                Anggota
                                            </span>
                                            <template x-if="team.pending_count > 0">
                                                <span class="count-pending"
                                                    x-text="`(${team.pending_count} pending)`"></span>
                                            </template>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <span :class="`badge-status badge-${team.status.toLowerCase()}`"
                                        x-text="team.status"></span>
                                </td>
                                <td>
                                    <div class="cell-actions">
                                        <button type="button" class="action-btn" title="View Detail"
                ...
                <div class="pagination-btns">
                    ...
                    <div class="page-numbers">
                        <template x-for="page in totalPages" :key="page">
                            <button class="page-num-btn" :class="{ 'active': currentPage === page }" @click="goToPage(page)"
                                x-text="page"></button>
                        </template>
                    </div>
        ...
        <!-- Toast Notifications -->
        <div class="toast-container" x-show="toast.show" x-transition:enter="toast-enter" x-transition:leave="toast-leave"
            style="display: none;">
