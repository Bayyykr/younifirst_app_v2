<aside class="sidebar" :class="{ 'collapsed': collapsed }">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="{{ route('admin.users') }}" class="nav-item {{ request()->routeIs('admin.users') ? 'active' : '' }}">
            ...
            <span x-show="!collapsed" x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">User Management</span>
        </a>
        <a href="{{ route('admin.events') }}" class="nav-item {{ request()->routeIs('admin.events') ? 'active' : '' }}">
            <i data-lucide="calendar"></i>
            <span x-show="!collapsed" x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">Event Management</span>
        </a>
